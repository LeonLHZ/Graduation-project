	...
	$.post("${pageContext.request.contextPath}/super?method=findAllAnnouncement",function(data){
		$(data).each(function(){
			$("#divhtm").append(
					...
					"<div>发布时间：<span>"+this.releasetime+"</span>发布人：<span>"+this.superadmin.name+"</span><a href='${pageContext.request.contextPath}/super?method=delete&aid="+this.annid+"'>删除</a></div></div>"
			);
		});
	},"json");
});
